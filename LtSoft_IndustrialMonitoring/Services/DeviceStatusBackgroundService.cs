...
namespace LtSoft_IndustrialMonitoring.Services
{
	...
	public class DeviceStatusBackgroundService : BackgroundService
	{
		...
		// 告警抑制：同一设备在冷却期内不重复发送相同类型告警
		private readonly ConcurrentDictionary<int, DateTime> _lastAlertTimestamps = new();
			...
			while (!stoppingToken.IsCancellationRequested)
			{
				try
				{
                    ...
                    foreach (Device device in devices)
					{
						...
						// 使用返回值作为最终判定依据，避免依赖本地对象状态不同步
						bool isOnline = await deviceService.CheckDeviceStatusAsync(device.Id);

						if (previousStatus && !isOnline)
						{
							// 告警冷却检查
							if (_lastAlertTimestamps.TryGetValue(device.Id, out DateTime last) && (DateTime.UtcNow - last) < _alertCooldown)
							{
								_logger.LogInformation($"已在冷却期内，跳过掉线告警: {device.Name} ({device.IP}:{device.Port})");
							}
							else
							{
								_lastAlertTimestamps[device.Id] = DateTime.UtcNow;
						...
						else if (!previousStatus && isOnline)
						{
							if (_lastAlertTimestamps.TryGetValue(device.Id, out DateTime last) && (DateTime.UtcNow - last) < _alertCooldown)
							{
								_logger.LogInformation($"已在冷却期内，跳过恢复告警: {device.Name} ({device.IP}:{device.Port})");
							}
							else
							{
								_lastAlertTimestamps[device.Id] = DateTime.UtcNow;
		...
		private async Task SendDingTalkAlertAsync(List<string> messages)
		{
			try
			{
				...
				//string? webhookUrl3 = configuration.GetValue<string>("DingTalk:WebhookUrl3");

				if (string.IsNullOrEmpty(webhookUrl1) /*&& string.IsNullOrEmpty(webhookUrl2) && string.IsNullOrEmpty(webhookUrl3)*/)
				{
