...
namespace LtSoft_IndustrialMonitoring.Services
{
	...
	public class DeviceService : IDeviceService
	{
		...
		public async Task<bool> CheckDeviceStatusAsync(int deviceId)
		{
			...
			bool previous = device.IsOnline;

			bool isOnline = await _communicationService.CheckDeviceStatus(device);

			// Use UTC for LastCommunication to keep logic timezone-agnostic
			...
			{
				// Only update LastCommunication to avoid unnecessary writes to other columns
				_context.Entry(device).Property(d => d.LastCommunication).IsModified = true;
				await _context.SaveChangesAsync();
			}
